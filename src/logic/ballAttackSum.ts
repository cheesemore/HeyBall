...
export function sumAttackFromLaunchUnits(units: LaunchBallUnit[]): number {
  let total = 0;
  for (const u of units) {
    total += getBallCombatStats(u.color, u.isBig).attack;
  }
  return total;
}

export function sumAttackFromSlots(slots: (BallItem | null)[]): number {
  const units = [];
  for (const item of slots) {
    if (item) units.push(...expandCellToLaunchUnits(item));
  }
  return sumAttackFromLaunchUnits(units);
}
